...
- Updated QS to 1.4.44 so WebBrowser preview secondary search rewrites the preview through a dedicated document helper, highlights visible body content, marks the first HTML body hit for scrolling, and uses a `Find` button with an embedded magnifying-glass bitmap instead of an emoji-only button.
