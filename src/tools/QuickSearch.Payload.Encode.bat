...
if "%QS_PAYLOAD_OUTPUT_PATH%"=="" (
	if not exist "%RELEASE_DIR%" mkdir "%RELEASE_DIR%"
	set "OUTPUT_PATH=%RELEASE_DIR%\%PROJECT_VERSION%-payload.txt"
) else (
	set "OUTPUT_PATH=%QS_PAYLOAD_OUTPUT_PATH%"
)

for %%I in ("%OUTPUT_PATH%") do set "OUTPUT_DIR=%%~dpI"
if not exist "%OUTPUT_DIR%" mkdir "%OUTPUT_DIR%"

echo Encoding QuickSearch Brotli Base64 payload...
echo Output: "%OUTPUT_PATH%"
pwsh.exe -NoProfile -ExecutionPolicy Bypass -File "%PAYLOAD_SCRIPT%" -OutputPath "%OUTPUT_PATH%"
if errorlevel 1 goto failed

for %%I in ("%OUTPUT_PATH%") do set "PAYLOAD_CHARACTERS=%%~zI"
echo.
echo Payload encoded successfully.
echo File: "%OUTPUT_PATH%"
echo Characters: %PAYLOAD_CHARACTERS%
goto done

:failed
echo.
echo Payload encode failed.
